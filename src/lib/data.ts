...
export async function getAllUsers(): Promise<User[]> {
  ...
  if (
    ...
    if (bookingError) {
      console.error(
        "Error fetching bookings for staff user scoping:",
        bookingError
      );
      return [];
    }
 ...
 */
export async function getListingTypesWithSampleImages(): Promise<
...
> {
  ...
  for (const listingType of uniqueTypes) {
    ...
    if (listingsError) {
      console.error(
        `Error fetching listings for type ${listingType}:`,
        listingsError
      );
      continue;
    }

    const allImages = listingsForType.flatMap(
      (listing) => (listing.data.images as string[]) || []
    );
 ...
 */
export async function getInventoryByListingId(
  listingId: string
...
export async function getAllBookings(options?: {
  ...
  let query = supabase
    .from("bookings")
    .select(
      "id, listing_id, user_id, status, start_date, end_date, data, listing:listings!inner(location)"
    );
  ...
  if (options?.fromDate && options?.toDate) {
    ...
    query = query
      .lte("start_date", options.toDate)
      .gte("end_date", options.fromDate);
  }
  ...
  const allInventoryIds = [
    ...new Set(unpackedBookings.flatMap((b) => b.inventoryIds || [])),
  ];
  ...
  if (usersResult.error)
    console.error("Error fetching user names for bookings:", usersResult.error);
  if (listingsResult.error)
    console.error(
      "Error fetching listing names for bookings:",
      listingsResult.error
    );
  if (inventoryResult.error)
    console.error(
      "Error fetching inventory names for bookings:",
  ...
  const inventoryMap = new Map(
    inventoryResult.data?.map((i) => [i.id, i.name])
  );
  ...
  mappedBookings.sort((a, b) => {
    ...
    const nameComparison = (a.bookingName || "").localeCompare(
      b.bookingName || ""
    );
...
export async function getBookingById(id: string): Promise<Booking | null> {
  ...
  if (session.role === "staff" && !canReadAny) {
    const canReadThisBooking = session.listingIds?.includes(
  ...
  if (
    unpackedBooking.inventoryIds &&
    Array.isArray(unpackedBooking.inventoryIds) &&
    unpackedBooking.inventoryIds.length > 0
  ) {
...
export async function getFilteredListings(
  ...
  const to = filters.date.to || filters.date.from;
  ...
  for (const booking of overlappingBookings) {
    ...
    (booking.data.inventoryIds || []).forEach((invId: string) =>
      bookedUnitsByListing[booking.listing_id].add(invId)
    );
  }
...
export async function getBookingsByDateRange(
  ...
  if (usersResult.error)
    console.error("Error fetching user names for report:", usersResult.error);
  if (inventoryResult.error)
    console.error(
      "Error fetching inventory names for report:",
      inventoryResult.error
    );
